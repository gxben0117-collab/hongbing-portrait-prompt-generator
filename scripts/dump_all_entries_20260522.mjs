/**
 * 輸出所有分類條目到 batch_entries_review.md
 * 每個分類按 ID 數字排序，最高 ID（最新批次）在最後。
 * 帶 [TMPL] 標記的是明顯模板殘留（名稱含 anchor 重複字詞）。
 */
...
// R5 的完整 anchor 字串（直接來自 CATEGORY_CONFIG）
const ANCHORS = {
...
const lines = [];
lines.push('# 風格範例全量審查清單（含批次標記）');
lines.push('');
lines.push('`[R5]` = 第五批 anchor 辨識命中 | `[TMPL]` = 名稱含模板殘留（anchor 字重複）');
lines.push('');

let r5Count = 0, tmplCount = 0;

for (const cat of cats) {
  const anchors = ANCHORS[cat.id] || [];
  ...
  lines.push(`## [${cat.id}] ${cat.label || ''} — ${cat.entries.length} 組`);

  for (const e of sorted) {
    const nameAndSub = `${e.name} · ${e.sub}`;
    const matchedAnchor = anchors.find(a => nameAndSub.includes(a));
    const isR5 = !!matchedAnchor;
    // 模板殘留偵測：名稱直接以 anchor 開頭（如「汝窯案几書閣焚香」「林場石徑林徑馭風」）
    const isTmpl = isR5 && matchedAnchor && e.name.startsWith(matchedAnchor);

    let tag = '';
    if (isTmpl) { tag = '[TMPL] '; tmplCount++; }
    else if (isR5) { tag = '[R5]   '; r5Count++; }
    else { tag = '       '; }

    lines.push(`  ${tag}${e.id.padEnd(12)} "${e.name}" · "${e.sub}"`);
  }
  lines.push('');
}

lines.push(`---`);
lines.push(`R5 命中：${r5Count} 組 | 模板殘留 [TMPL]：${tmplCount} 組`);

const outPath = path.join(__dirname, 'batch_entries_review.md');
fs.writeFileSync(outPath, lines.join('\n'), 'utf8');
console.log(`輸出至: ${outPath}`);
console.log(`R5 命中：${r5Count} | TMPL：${tmplCount}`);
